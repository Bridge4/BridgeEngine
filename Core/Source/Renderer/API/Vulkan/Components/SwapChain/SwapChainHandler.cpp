...
void SwapChainHandler::Initialize() {
    ...
    QueueFamilyIndices indices = deviceHandler->FindQueueFamilies(
        m_vulkanGlobalState->GetPhysicalDevice());
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        // static function
        swapChainImageViews[i] = imageViewBuilder->CreateImageView(
...
void SwapChainHandler::Destroy() {
    vkDestroyImageView(*m_vulkanGlobalState->GetRefLogicalDevice(),
                       m_vulkanGlobalState->m_depthImageView, nullptr);
    vkDestroyImage(*m_vulkanGlobalState->GetRefLogicalDevice(),
                   m_vulkanGlobalState->m_depthImage, nullptr);
    vkFreeMemory(*m_vulkanGlobalState->GetRefLogicalDevice(),
                 m_vulkanGlobalState->m_depthImageMemory, nullptr);
...
void SwapChainHandler::CreateDepthResources() {
    ...
    CreateImage(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_vulkanGlobalState->m_depthImage,
        m_vulkanGlobalState->m_depthImageMemory);
    m_vulkanGlobalState->m_depthImageView = imageViewBuilder->CreateImageView(
        *m_vulkanGlobalState->GetRefLogicalDevice(),
        m_vulkanGlobalState->m_depthImage, depthFormat,
        VK_IMAGE_ASPECT_DEPTH_BIT);

    TransitionImageLayout(m_vulkanGlobalState->m_depthImage, depthFormat,
...
void SwapChainHandler::CreateFramebuffers() {
    CreateDepthResources();
    m_vulkanGlobalState->m_swapChainFramebuffers.resize(
    ...
    for (size_t i = 0; i < m_vulkanGlobalState->m_swapChainImageViews.size();
        ...
        if (vkCreateFramebuffer(
                *m_vulkanGlobalState->GetRefLogicalDevice(), &framebufferInfo,
                nullptr, &m_vulkanGlobalState->m_swapChainFramebuffers[i]) !=
            VK_SUCCESS) {
...
// Helpers
void SwapChainHandler::CreateImage(uint32_t width, uint32_t height,
...
VkFormat SwapChainHandler::FindSupportedFormat(
...
uint32_t SwapChainHandler::FindMemoryType(uint32_t typeFilter,
                                          VkMemoryPropertyFlags properties) {
...
void SwapChainHandler::TransitionImageLayout(VkImage image, VkFormat format,
