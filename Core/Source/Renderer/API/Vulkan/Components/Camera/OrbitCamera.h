...
// Source:
// https://www.mbsoftworks.sk/tutorials/opengl4/026-camera-pt3-orbit-camera/#:~:text=Orbit%20camera%20(also%20known%20as,just%20floating%20around%20the%20sphere.
...
class OrbitCamera {
   public:
    OrbitCamera(const glm::vec3& center, const glm::vec3& upVector,
                float radius, float minRadius, float azimuthAngle,
                float polarAngle) {
        m_center = center;
        m_upVector = upVector;
        m_radius = radius;
   ...
   private:
    glm::vec3 m_center;    // Center of the orbit camera sphere (the point upon
                           // which the camera looks)
    glm::vec3 m_upVector;  // Up vector of the camera
    float m_radius;        // Radius of the orbit camera sphere
    float m_minRadius;     // Minimal radius of the orbit camera sphere (cannot
