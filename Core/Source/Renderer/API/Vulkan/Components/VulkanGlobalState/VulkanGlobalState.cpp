...
#include "Source/Renderer/API/Vulkan/VulkanContext.h"
...
void VulkanGlobalState::CreateVulkanInstance() {
    ...
    // Telling Vulkan what global (program) extensions and validations we want
    // to use
    ...
    std::vector<const char*> extensions =
        getRequiredExtensions(&m_vulkanContext->enableValidationLayers);
    ...
    if (m_vulkanContext->enableValidationLayers) {
        createInfo.enabledLayerCount =
            static_cast<uint32_t>(m_vulkanContext->m_validationLayers.size());
        createInfo.ppEnabledLayerNames =
            m_vulkanContext->m_validationLayers.data();
